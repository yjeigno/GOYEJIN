...
/**
  ...
  * 테스트할 가볍게 하기 위해서 @WebMvcTest 를 사용해서 web 레이어 관련 빈들만 등록한 상태로 테스트를 할 수도 있다. 단, web레이어 관련된 빈들만 등록되므로 Service는 등록되지 않는다. 그래서 Mock 관련 어노테이션을 이용해서 가짜로 만들어줘야 한다.
 ...
 간단히 말하면, 내가 컨트롤러 테스트 하고 싶을때 실제 서버에 올리지 않고 테스트 용으로 시뮬레이션 해서 MVC가 되도록 해주는 클래스
  *그냥 컨트롤러 슬라이스 테스트 한다고 하면 @WebMvcTest 랑 MockMvc 쓰면 됨

 2) @DataJpaTest - JPA 레포지토리 테스트 할때 사용
 @Entity가 있는 엔티티 클래스들을 스캔해서 테스를 위한 JPA 레포지토리들을 설정
...
public class Ex07_3_2_DataRestTest_성공하는_테스트 {
    ...
    public Ex07_3_2_DataRestTest_성공하는_테스트(@Autowired MockMvc mvc) { /* 2) MockMvc 에게 요청에 대한 정보를 입력(주입) */
        this.mvc = mvc;
    }
...
//                .andExpect(content().contentType(MediaType.valueOf("application/hal+json")));
         ...
         *static import란 필드나 메서드를 클래스를 지정하지 않고도 코드에서 사용할 수 있도록 하는 기능.
         ...
         valueOf 안에 들어갈 content-type은 아까 HAL 의 Response Headers 에 있는 content-type 에 있는거 복사해오기
         * */
    }
}
