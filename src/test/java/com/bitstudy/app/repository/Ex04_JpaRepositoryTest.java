...
class Ex04_JpaRepositoryTest {
    ...
    * */
    ...
    @Test
    void insertTest() {
        /** 기존의 article 개수를 센 다음에, insert 하고, 기존 것보다 현재 거가 1 차이가 나면 insert 제대로 됐다는 뜻. */
    ...
    @Test
    void update() {
        /** 기존의 데이터 하나 있어야 되고, 그걸 수정했을 때 관찰할 거
         *
         * 1) 기존의 영속성 컨텍스트로부터 하나 엔티티 객체를 가져온다. (DB에서 한 줄 뽑아 온다)
         * 2) 업데이트로 해시태그를 바꾸기
         * */

        /* 순서 1) 기존의 영속성 컨텍스트로부터 하나 엔티티 객체를 가져온다. (DB에서 한 줄 뽑아 온다)
            articleRepository -> 기존의 영속성 컨텍스트로부터
            findById(1L) -> 하나 엔티티를 가져온다
            orElseThrow() -> 없으면 일단 throw 시켜서 일단 테스트가 끝나게 하자
        */
...
//        articleRepository.save(article);
        ...
        /** save로 놓고 테스트를 돌리면 콘솔(Run) 탭에 update 구문이 나오지 않고 select 구문만 나온다. 이유는 영속성 컨텍스트로부터 가져온 데이터를 그냥 save만 하고 아무것도 하지 않고 끝내버리면 어차피 롤백 되니까 스프링부트는 다시 원래의 값으로 돌아가질 거다. 그래서 그냥 했다치고 update 를 하지 않는다.(코드의 유효성은 확인)
         그래서 save를 하고 flush 해 줘야 한다.
        ...
        /* 순서 3) 위에서 바꾼 savedArticle 에 업데이트 된 hashtag 필드에 updateHashtag 에 저장되어 있는 값("#abcd")이 있는지 확인해 봐라
         * hasFieldOrPropertyWithValue -> 해당 필드에 값이 있냐?
         * */
    ...
    @Test
    void delete() {
        /** 기존의 데이터들이 있다고 치고, 그 중에 값을 하나 꺼내고 지워야 한다.
         *
         * 1) 기존의 영속성 컨텍스트로부터 하나 엔티티 객체를 가져온다. => findById
         * 2) 지우면 DB에서 하나 사라지기 때문에 count 를 구해 놓고 => 레포지토리.count()
         * 3) delete 하고(-1) => .delete
         * 4) 2번에서 구한 count와 지금 순간의 개수 비교해서 1 차이 나면 테스트 통과 => .isEqualTo
         * */
        ...
        */
        ...
        /* 2) 지우면 DB에서 하나 사라지기 때문에 count 를 구해 놓고
         * 게시글(articleRepository) 뿐만 아니라, 연관된 댓글(articleCommentRepository) 까지 삭제할 거라서 두 개의 개수를 다 알아내기.
         *
         * */
        ...
        int deletedCommentSize = article.getArticleComments().size(); // 해당 게시글에 딸려씨는 댓글의 수
        // 나중에 "모든 댓글의 수 = 게시글에 딸려있는 댓글 수" 하면 몇 개 지워졌는지 알 수 있다.

        /* 3) delete 하고 (전체 게시글 개수 -1 됨 */
        articleRepository.delete(article);

        /* 테스트 통과 조건 - 2번에서 구한 거랑 여기서 구하는 개수가 1 차이 나는 경우 */
